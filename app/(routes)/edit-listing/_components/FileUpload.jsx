...
function FileUpload({setImages, imageList}) {
    ...
    return (
        ...
        <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-7 xl:grid-cols-8 mt-3 gap-3'>
            {imagePreview.map((image, index)=>(
                <Image key={index} src={image} width={200} height={200} className='rounded-lg object-cover h-[100px] w-[100px]' alt="Preview"/>
            ))    
            }
        </div>
        {imageList && <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-7 xl:grid-cols-8 mt-3 gap-3'>
            {imageList.map((image, index)=>(
                <Image key={index} src={image?.url} width={200} height={200} className='rounded-lg object-cover h-[100px] w-[100px]' alt="Preview"/>
            ))    
            }
        </div>}
        </div>
